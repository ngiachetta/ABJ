...
```{r}
file <- dir("TJRJ_Data/data-raw/", full.names = T)

obter_fase_atual <- function(file){
  node %>% rvest::html_nodes(xpath = '//table[@width="100%"][3]') %>%
...
parse_movimentacao <- purrr::possibly(parse_movimentacao, otherwise = dplyr::tibble(result = "ok"))

#parse_movimentacao(files[1])
# -------------------------------------------------------------------------------------------------
parse_info <- function(file){
  ...
  info <- node %>% rvest::html_nodes(xpath = '//table[@width="100%"][3]') %>%
    rvest::html_table(fill = T) %>% 
    purrr::pluck(1) %>% 
    dplyr::select(1, 2) %>% 
    magrittr::set_names(c("key", "val")) %>% 
...
# -------------------------------------------------------------------------------------------------
tab <- purrr::map_dfr(file, ~parse_tjrj(.x))
```
