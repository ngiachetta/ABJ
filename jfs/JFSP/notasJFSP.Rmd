...
jfsp_selenium <- function(query){
  ...
  TodasFases <- TodasFases %>% rvest::html_table(header = T, fill = T) %>% dplyr::last()

  DetalheFases <- DetalheFases[[1]] %>%
    rvest::html_table(fill = T, trim = T, header = T) %>%
    dplyr::last() %>%
    ...
    t() %>%
    tibble::as.tibble() %>%
    mutate(X1 = stringr::str_replace(X1, "Consulta da Movimentação Número : ", "") %>% stringr::str_trim())
...
```{r}
Parsing
 Page1 <- info_geral(page1 = infos$Page_1)
  # 
  TodasPeticoes <- infos$Todas_Peticoes %>%
    rvest::html_table(header = T, fill = T) %>% dplyr::last() %>%
    mutate(Data = lubridate::dmy(Data),
           Processo = query)

  TodasFases <- infos$Todas_Fases %>% rvest::html_table(header = T, fill = T) %>% dplyr::last()
  # 
  DetalheFases <- infos$Todas_Fases %>%
    rvest::html_table(fill = T, trim = T, header = T) %>%
    dplyr::last() %>%
    magrittr::set_names(c("X1", "X2")) %>%
    dplyr::select(X2) %>%
    dplyr::mutate(X2 = stringr::str_trim(X2),
                  X2 = ifelse(X2 == "", NA, X2)) %>%
    tidyr::drop_na() %>%
